...
//
// {  body: '{    "supporterName" : "name",   "amount" : "19.00"   }'
//
// }
   ...
   // ready to go for CORS. To make this a completed HTTP response, you only need to add a statusCode and a body.
    ...
    try {
        ...
        if(fetchedWallet != null){
            let resultUpdateWallet = await updateWallet(fetchedWallet + parseInt(info.amount), info.supporterName); 
            //update wallet (add the funds)
            
            if(resultUpdateWallet!= null) {
                response.statusCode = 200;
                let status = await insertDonation("addFunds", info.amount, info.supporterName, null);
                //projectName, amount, supporterName, pledgeName
                
                if(status === 200) {
                    response.statusCode = 200;
                    response.newWallet = resultUpdateWallet;
                    response.msg = "Funds added successfully"
                    //insertion successful
                } else {
                    response.statusCode = 400;
                    response.error = "Couldn't make donation for addFunds"
                }
            } else {
                response.statusCode = 400;
